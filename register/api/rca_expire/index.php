...
function GUID() {
   ...
   $sql = "SELECT rn, rca,datediff(rca,now()) as rcadiff FROM `autobook_vin` WHERE (rca IS NOT NULL) and (year(rca)>'2000') and datediff(rca,now())<=7";
   $result = $con -> query($sql);
    ...
    if ($result->num_rows > 0) {
		 while ($rws=$result->fetch_assoc()){
			  ...
			  $rca_date = $rws['rca'];
			  $days		= $rws['rcadiff'];
			  $text = "Pentru autovehicolul ".$auto." RCA ";
			  if ($days=='0') {$text.= " expira azi ".$rca_date;}
